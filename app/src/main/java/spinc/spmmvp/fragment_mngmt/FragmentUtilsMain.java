...
public class FragmentUtilsMain {
	...
	public void popBackStack(boolean isAnimation) {
	...
	public void saveFragmentState(Fragment fragment){
		//Save the current instance state of the given Fragment.
		getFragmentManager().saveFragmentInstanceState(fragment);
	}
	public void clearBackStackLoadSingleFragment(Fragment fragment, int containerId, boolean backStack) {
		FragmentManager manager = getFragmentManager();
		if (manager.getBackStackEntryCount() > 0) {
			manager.popBackStack(null, FragmentManager.POP_BACK_STACK_INCLUSIVE);
		}
		startFragment(fragment,containerId,backStack);
	}
